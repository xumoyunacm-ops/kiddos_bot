...
if __name__ == "__main__":
    ...
    # Сбрасываем возможный вебхук и накопившиеся апдейты —
    # помогает, если раньше бот падал с ошибкой 409 (Conflict).
    try:
        bot.remove_webhook()
    except Exception as e:
        log.warning("remove_webhook: %s", e)

    log.info("Kiddos bot запущен...")

    # Устойчивый цикл: если Telegram вернёт временный конфликт (409),
    # бот подождёт и попробует снова, а не упадёт.
    while True:
        try:
            # skip_pending НЕ используем — именно он раньше ронял бота на старте
            bot.polling(none_stop=True, interval=1, timeout=30)
        except Exception as e:
            log.error("Polling упал: %s — перезапуск через 5 сек", e)
            time.sleep(5)
